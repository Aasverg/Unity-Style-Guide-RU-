...
> 3.  [Сценарии](#scripts)
> 4.  [Соглашения об именовании ассетов](#anc)
> 5.  [Рабочие процессы управления ассетами](#asset-workflows)
...
*   `Dead` — это значит, что он мёртвый? или что он умрёт?
*   `Rock`
...
Большинство вещей имеют префикс, который обычно представляет собой аббревиатуру типа ассета, за которой следует нижнее подчеркивание.

**В ресурсах используется [PascalCase.](#cases)**

<a name="base-asset-name"></a>
<a name="4.1"></a>
### 4.1 Название базового ассета - `Prefix_BaseAssetName_Variant_Suffix`

Все ассеты должны иметь *базовое название ассета* . Базовое название ассета представляет собой логическую группу связанных ассетов. Любой ассет, входящий в эту логическую группу, должен соответствовать стандарту `Prefix_BaseAssetName_Variant_Suffix` .

Как правило, для создания хороших названий ассетов достаточно помнить о шаблоне `Prefix_BaseAssetName_Variant_Suffix` и руководствоваться здравым смыслом. Вот несколько подробных правил, касающихся каждого элемента.

`Prefix` и `Suffix` определяются типом ассета с помощью следующих таблиц [модификаторов названия ассета](#asset-name-modifiers) .
...
При присвоении имени ассету используйте эти таблицы для определения префикса и суффикса, которые следует использовать с [базовым именем](#base-asset-name) ассета.

#### Разделы

> 4.2.1 [Наиболее распространенное](#anc-common)
...
## 5\. Рабочие процессы управления ассетами
...
*   Следуйте [правилам именования ассетов.](#anc-models)
